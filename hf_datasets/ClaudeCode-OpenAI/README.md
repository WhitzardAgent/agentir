...
# ClaudeCode in OpenAI Chat Messages format
...
Standard `[{'role': 'user', 'content': ...}, ...]` JSONL with native `tool_calls` and `tool` role messages.
...
agentir-llc --input canonical.air.jsonl --target openai-tools --output output.jsonl
```
...
ds = load_dataset("agentir/ClaudeCode-OpenAI", split="train")
print(ds[0])
```
